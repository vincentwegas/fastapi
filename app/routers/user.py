...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    #Feature -> Funktion die erkennt, das es den User bereits gibt!
...
def get_user(id: int , db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail =f"User with id: {id} not found and doesn't exists ")
